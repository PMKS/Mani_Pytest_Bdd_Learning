...
@given(parsers.cfparse('The Basket has "{initial:Number}" items',extra_types=EXTRA_TYPE))
...
@when(parsers.cfparse('"{some:Number}" items are added in to Basket',extra_types=EXTRA_TYPE))
def items_are_added_in_to_basket(basket4,some):
    """"4" items are added in to Basket."""
    basket4.addItem(some)


@then(parsers.cfparse('The Basket contains in "{final:Number}" Basket',extra_types=EXTRA_TYPE))
...
@when(parsers.cfparse('"{some:Number}" items are removed from the Basket',extra_types=EXTRA_TYPE))
def items_are_removed_from_the_Basket(basket5,some):
    """"4" items are added in to Basket."""
    basket5.removeItem(some)


@then(parsers.cfparse('The Basket contains "{final:Number}" items',extra_types=EXTRA_TYPE))
